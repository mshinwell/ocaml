module Deps = struct
  ...
  let write fd b off len = output fd b off len
  let close_out fd = close_out fd
  type in_file = in_channel
  let open_in filename = open_in_bin filename
  let read fd b off len = input fd b off len
  let reset_in fd = seek_in fd 0
  let close_in fd = close_in fd
