...
expr:
  ...
  | LPAREN APPLY location expr exprlist machtype RPAREN
                { Cop(Capply($6, debuginfo ?loc:$3 ()), $4 :: List.rev $5) }
  | LPAREN EXTCALL STRING exprlist machtype RPAREN
                { Cop(Cextcall($3, $5, false, debuginfo ()), List.rev $4) }
  | LPAREN ALLOC exprlist RPAREN { Cop(Calloc, List.rev $3) }
...
binaryop:
  ...
  | GTF                         { Ccmpf Cgt }
  | GEF                         { Ccmpf Cge }
  | CHECKBOUND                  { Ccheckbound (debuginfo ()) }
  | MULH                        { Cmulhi }
;
sequence:
