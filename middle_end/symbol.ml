...
module Predef = struct
  ...
  let startup = CU.create (CU.Name.of_string "_startup")
  let shared_startup = CU.create (CU.Name.of_string "_shared_startup")
  let runtime_and_external_libs = CU.create (CU.Name.of_string "_system")
  let functions = CU.create (CU.Name.of_string "_hot")
...
let name_for_backend t =
  ...
  | Lifted_variable { compilation_unit; name; stamp } ->
      Printf.sprintf "%s__%s_%d"
        (prefix_for_compilation_unit compilation_unit) name stamp
  | Lifted_closure { compilation_unit; name; stamp } ->
      Printf.sprintf "%s__%s_closure_%d"
        (prefix_for_compilation_unit compilation_unit) name stamp
  | Lifted_anonymous_constant { compilation_unit; stamp } ->
      Printf.sprintf "%s__%d"
        (prefix_for_compilation_unit compilation_unit) stamp
  | Code_pointer { compilation_unit; name; stamp } ->
      Printf.sprintf "%s__%s_%d"
        (prefix_for_compilation_unit compilation_unit) name stamp
  | Predefined_exn name -> "caml_exn_" ^ name

let make_backend_symbol cu s =
  match s with
    None -> prefix_for_compilation_unit cu
  | Some s ->
      Format.sprintf "%s__%s" (prefix_for_compilation_unit cu) s
