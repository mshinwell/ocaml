...
let param_types_and_body_env_opt cont_uses freshening ~default_env =
  ...
  | uses ->
    ...
    let arg_tys_with_env_extensions_rev, joined_env_extension =
      List.fold_left
        (fun (arg_tys_with_env_extensions_rev, joined_env_extension) use ->
          ...
          in
          (arg_tys, use_env_extension) :: arg_tys_with_env_extensions_rev,
            Some joined_env_extension)
        ([], None)
        uses
    in
