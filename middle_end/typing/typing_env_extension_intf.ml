...
(* CR mshinwell: Fix ocamldep or find some workaround so we can
   [open Types] below without introducing erroneous dependencies. *)
...
module type S = sig
  module T : Typing_world_abstract.S
  module Functor_T : Typing_world_abstract.Functor_S
  include module type of struct include T.Typing_env_extension end
  ...
  val add_definition_at_beginning : t -> Name.t -> T.Flambda_types.t -> t
  ...
  (** Add an equation (on a name that is either existentially bound in the
      ...
      the given extension. *)
  val add_equation : t -> Name.t -> T.Flambda_types.t -> t
