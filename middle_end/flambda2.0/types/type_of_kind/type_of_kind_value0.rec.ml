...
type t =
  ...
  | Tagged_immediate of Type_grammar.t
  | Boxed_float of T.t
  | Boxed_int32 of T.t
  | Boxed_int64 of T.t
...
let print_with_cache ~cache ppf t =
  ...
  | Tagged_immediate untagged_ty ->
    Format.fprintf ppf "@[<hov 1>(Tagged_immediate@ %a)@]"
      (T.print_with_cache ~cache) untagged_ty
...
let free_names t =
  ...
  | Tagged_immediate ty -> T.free_names ty
...
struct
  ...
  let meet_or_join env t1 t2 : _ Or_bottom_or_absorbing.t =
    ...
    | Tagged_immediate n1, Tagged_immediate n2 ->
      Or_bottom_or_absorbing.of_or_bottom
        (E.switch T.meet T.join env n1 n2)
        ~f:(fun (n, env_extension) -> Tagged_immediate n, env_extension)
