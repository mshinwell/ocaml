...
let erase_aliases_unknown_or_join erase_aliases_contents env ~bound_name
      ~allowed (o : _ Flambda_types.unknown_or_join)
      : _ Flambda_types.unknown_or_join =
  match o with
  | Unknown | Bottom -> o
  | Ok contents ->
    Ok (erase_aliases_contents env ~bound_name ~allowed contents)

let erase_aliases_ty env ~bound_name ~allowed erase_aliases_of_kind_foo
      ~force_to_kind:_ ~print_ty:_ ~apply_rec_info:_
      (ty : _ Flambda_types.ty) : _ Flambda_types.ty =
  match ty with
  | No_alias unknown_or_join ->
    No_alias (erase_aliases_unknown_or_join erase_aliases_of_kind_foo env
      ~bound_name ~allowed unknown_or_join)
  | Type _export_id -> ty
  | Equals simple ->
    ...
    (* CR mshinwell: Second part removed for the moment -- it will need an
       environment like [Meet_env] to not go round in circles on closure
       types *)
    let bound_name =
...
let erase_aliases_of_kind_naked_number (type n) _env ~bound_name:_ ~allowed:_
      (ty : n Flambda_types.of_kind_naked_number) = ty

let rec erase_aliases env ~bound_name ~allowed (t : Flambda_types.t)
      : Flambda_types.t =
  match t with
  | Value ty ->
    Value (
      erase_aliases_ty env ~bound_name ~allowed
...
and erase_aliases_of_kind_value env ~bound_name ~allowed
...
and erase_aliases_of_kind_fabricated env ~bound_name:_ ~allowed
...
let erase_aliases_ty_value env ~bound_name ~allowed ty =
  erase_aliases_ty env ~bound_name ~allowed erase_aliases_of_kind_value
    ~force_to_kind:Flambda_type0_core.force_to_kind_value
    ~print_ty:Type_printers.print_ty_value
    ~apply_rec_info:Flambda_type0_core.apply_rec_info_of_kind_value
    ty

let erase_aliases_ty_fabricated env ~bound_name ~allowed ty =
  erase_aliases_ty env ~bound_name ~allowed erase_aliases_of_kind_fabricated
    ~force_to_kind:Flambda_type0_core.force_to_kind_fabricated
    ~print_ty:Type_printers.print_ty_fabricated
    ~apply_rec_info:Flambda_type0_core.apply_rec_info_of_kind_fabricated
    ty
