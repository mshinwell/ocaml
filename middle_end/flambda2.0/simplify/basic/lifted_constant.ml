...
type t = {
  ...
  bound_symbols : Flambda_static.Bound_symbols.t;
...
let print ppf { types; bound_symbols; static_part; } =
  Format.fprintf ppf "@[<hov 1>(\
    ...
    Flambda_static.Bound_symbols.print bound_symbols
    Flambda_static.Static_part.print static_part

let create types bound_symbols static_part =
  let being_defined =
    Flambda_static.Bound_symbols.being_defined bound_symbols
  in
...
let introduce t typing_env =
  ...
  Symbol.Map.fold (fun sym typ typing_env ->
      TE.add_symbol typing_env sym (T.erase_aliases typ ~allowed))
    t.types
    typing_env
