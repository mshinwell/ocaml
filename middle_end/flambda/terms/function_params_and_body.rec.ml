...
let create ?free_names_of_body ~return_continuation exn_continuation params
      ~dbg ~body ~my_closure =
  let is_my_closure_used : _ Or_unknown.t =
    match free_names_of_body with
    | None -> Unknown
    | Some free_names_of_body ->
      Known (Name_occurrences.mem_var free_names_of_body my_closure)
  in
  let my_closure =
