...
type t =
  | Non_recursive of {
      ...
      num_free_occurrences : Name_occurrences.Num_occurrences.t Or_unknown.t;
    }
...
let create_non_recursive ?free_names_of_body cont handler ~body =
  let num_free_occurrences : _ Or_unknown.t =
    match free_names_of_body with
    | None -> Unknown
    | Some free_names_of_body ->
      Known (Name_occurrences.count_continuation free_names_of_body cont)
  in
