...
let create_non_recursive_let_cont cont handler ~body ~free_names_of_body =
  Let_cont.create_non_recursive cont handler ~body ~free_names_of_body

let create_apply apply = Expr.create_apply apply
let create_apply_cont apply_cont = Expr.create_apply_cont apply_cont
let create_invalid () = Expr.create_invalid ()

let rebuild_invalid uacc ~after_rebuild =
  after_rebuild (create_invalid ()) uacc
...
let add_wrapper_for_fixed_arity_continuation uacc cont ~use_id arity ~around =
  ...
  | New_wrapper (new_cont, new_handler) ->
    ...
    create_non_recursive_let_cont new_cont new_handler ~body
