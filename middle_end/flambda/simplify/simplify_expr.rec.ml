  ...
  L.pattern_match let_expr ~f:(fun bindable_let_bound ~body ->
    ...
    let dacc =
      (* We've now left the scope of the defining expression, so the types
         ...
         simplification of the [body].  Just before doing that we have to
      ...
      DA.map_denv dacc ~f:(fun denv ->
        DE.add_lifted_constants denv new_lifted_constants)
    in
    let dacc =
      (* Since the environment in [dacc] will be in scope for all of the
         [body], there is no need for lifted constants to be in [r] during
    ...
    | Bindings { bindings_outermost_first = bindings; dacc = _; } ->
      ...
      (* Place definitions for any lifted constants that must be placed
         now (see comment above) instead of being floated up to the next
         [Let_symbol] binding. *)
      if not place_lifted_constants_immediately then body, user_data, uacc
      else
        let new_lifted_constants =
          new_lifted_constants
        ...
        (* As a consequence of the clearing of lifted constants in [r]
           ...
           the current binding are provided in [new_lifted_constants].
    ...
    | Shared { symbol; kind; } ->
      ...
      (* This is similar to the [Reified] case above, except that the
         ...
         existing symbol. *)
      let dacc =
        let var = Bindable_let_bound.must_be_singleton bindable_let_bound in
      ...
      in
      let bindings = [bindable_let_bound, defining_expr] in
...
= fun dacc apply ~callee's_code_id ~callee's_closure_id
  ...
  let wrapper_taking_remaining_args, dacc =
    ...
    let dacc =
      dacc
      |> DA.map_r
           ~f:(fun r -> R.add_still_to_be_placed_lifted_constant r dummy_code)
...
*)
    ...
    let create_tagged_scrutinee k =
      ...
      in
      match Simplify_named.simplify_named dacc bound_vars named with
      | Bindings
