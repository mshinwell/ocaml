    ...
    let free_names t =
      ...
      | Block_like { symbol_projections; _ } ->
        (* The symbols mentioned in any symbol projections must be counted
           ...
           in the code.  However the variables corresponding to those
           projections (which may occur in [t.defining_expr]) must not be
           counted, since we expect to be able to lift the definition past the
           existing bindings of such variables, and then rematerialise the
           bindings if required. *)
        Variable.Map.fold (fun var proj free_names ->
