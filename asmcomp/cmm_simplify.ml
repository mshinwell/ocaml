...
module Env : sig
  ...
  let rec eligible (expr : Cmm.expression) =
    ...
    | Cop (op, args, _dbg) ->
      ...
      | Clsl
      | Clsr
      | Casr -> Array.for_all (fun arg -> eligible arg) args
      | Capply _
      | Cextcall _
      | Cload _
      | Calloc
  ...
  let add t id (expr : Cmm.expression) =
    if eligible then Ident.Map.add id expr t
    else t
...
let add_no_overflow op n x c dbg =
  let d = n + x in
...
let rec add_const_generic op c n dbg =
  if n = 0 then c
    ...
    | Cop(Csubi, [Cconst_int x; c], _)
          when op = Caddi && no_overflow_add n x ->
        Cop(Csubi, [Cconst_int (n + x); c], dbg)
    | Cop(Csubi, [c; Cconst_int x], _)
          when op = Caddi && no_overflow_sub n x ->
        add_const op c (n - x) dbg
    | c -> Cop(op, [c; Cconst_int n], dbg)

let add_const c n dbg = add_const_generic Cmm.Caddi c n dbg

let rec add_int (op : Cmm.operation) env c1 c2 dbg =
  assert (op = Caddi || op = Cadda || op = Caddv);
...
let rec simplify_op env (op : Cmm.operation) args dbg expr =
  ...
  | Cmodi -> mod_int env args.(0) args.(1) dbg
  | Cand
  | Cor
  | Cxor -> expr
