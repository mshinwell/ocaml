...
(** Create an assembly symbol given its (unescaped) name, the section within
    which it is defined and the object file within which it is defined. The name
    will be prefixed with any required symbol prefix for the target system's
    assembler. No OCaml-specific conventions (such as prefixing with "caml", or
    whatever) will be applied to the name. *)
val of_external_name : Asm_section.t -> Object_file.t -> string -> t

(** Take an existing symbol and add a prefix to its name.  The [object_file]
    and section must be specified as they often change during a
    prefixing. *)
val prefix : t -> Asm_section.t -> Object_file.t -> prefix:string -> t
...
(** Like [of_external_name], but for specialised uses (in particular "direct
    ...
    applied. *)
val of_external_name_no_prefix : Asm_section.t -> Object_file.t -> string -> t
