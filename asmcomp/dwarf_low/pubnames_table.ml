...
(*
  let write_offset_name_pair function_name =
    ...
    Dwarf_value.emit (Dwarf_value.as_string function_name) asm
  in
  Dwarf_value.emit (Dwarf_value.as_four_byte_int (size t)) asm;
  Dwarf_value.emit (Dwarf_value.as_two_byte_int 2) asm;  (* version number *)
  Dwarf_value.emit (Dwarf_value.as_four_byte_int_from_label t.debug_info0) asm;
  Dwarf_value.emit
    (Dwarf_value.as_four_byte_int (Debug_info_section.size t.debug_info)) asm;
  List.iter t.externally_visible_functions ~f:write_offset_name_pair;
  Dwarf_value.emit (Dwarf_value.as_four_byte_int 0) asm
*)
