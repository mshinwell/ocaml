...
let rec div_int c1 c2 dbg =
  (* CR mshinwell: Fix formatting after code review *)
  ...
  | (c1, Cconst_int n) when n <> min_int ->
      ...
      if n = 1 lsl l then
        ...
        *)
        ...
        sub_int (Cconst_int 0) (div_int c1 (Cconst_int (-n)) dbg) dbg
      else begin
...
let safe_div_bi =
  safe_divmod_bi div_int (fun c1 dbg -> Cop(Csubi, [Cconst_int 0; c1], dbg))

let safe_mod_bi =
  safe_divmod_bi mod_int (fun _ _ -> Cconst_int 0)

(* Bool *)

let test_bool cmm dbg =
...
let complex_re c dbg = Cop(Cload Double_u, [c], dbg)
let complex_im c dbg = Cop(Cload Double_u,
...
let get_field ptr n dbg =
  Cop(Cload Word_val, [field_address ptr n dbg], dbg)
...
let get_header ptr dbg =
  Cop(Cload Word_int, [Cop(Cadda, [ptr; Cconst_int(-size_int)], dbg)], dbg)
...
let addr_array_ref arr ofs dbg =
  Cop(Cload Word_val, [array_indexing log2_size_addr arr ofs dbg], dbg)
let int_array_ref arr ofs dbg =
  Cop(Cload Word_int, [array_indexing log2_size_addr arr ofs dbg], dbg)
let unboxed_float_array_ref arr ofs dbg =
  Cop(Cload Double_u, [array_indexing log2_size_float arr ofs dbg], dbg)
...
let lookup_label obj lab dbg =
  bind "lab" lab (fun lab ->
    let table = Cop (Cload Word_val, [obj], dbg) in
...
let split_int64_for_32bit_target arg dbg =
  bind "split_int64" arg (fun arg ->
    ...
    Ctuple [Cop (Cload Thirtytwo_unsigned, [first], dbg);
            Cop (Cload Thirtytwo_unsigned, [second], dbg)])

let rec unbox_int bi arg dbg =
  preserve_phantom_lets_cmm arg (fun arg ->
  match arg with
  | Cop(Calloc, [_hdr; _ops; arg], _) ->
      begin match strip_phantom_lets_cmm arg with
      | Cop (Clsl, [contents; shift_by_32], _)
...
let bigarray_indexing unsafe elt_kind layout b args dbg =
  ...
  | [arg] ->
      if unsafe then arg
        ...
        bind "idx" arg (fun idx ->
          ...
          let bound = Cop(Cload Word_int,[field_address b dim_ofs dbg], dbg) in
  ...
  array_indexing ~typ:Int (log2 elt_size)
                 (Cop(Cload Word_int, [field_address b 1 dbg], dbg)) offset dbg
    ...
    match elt_kind with
      Pbigarray_complex32 | Pbigarray_complex64 ->
        ...
        bind "addr" (bigarray_indexing unsafe elt_kind layout b args dbg)
          (fun addr ->
          box_complex dbg
            (Cop(Cload kind, [addr], dbg))
            (Cop(Cload kind, [Cop(Cadda, [addr; Cconst_int sz], dbg)], dbg)))
    | _ ->
...
let unaligned_load_16 ptr idx dbg =
  ...
  then Cop(Cload Sixteen_unsigned, [add_int ptr idx dbg], dbg)
  else
    let v1 = Cop(Cload Byte_unsigned, [add_int ptr idx dbg], dbg) in
...
let unaligned_load_32 ptr idx dbg =
  if Arch.allow_unaligned_access
  then Cop(Cload Thirtytwo_unsigned, [add_int ptr idx dbg], dbg)
  else
    let v1 = Cop(Cload Byte_unsigned, [add_int ptr idx dbg], dbg) in
    let v2 = Cop(Cload Byte_unsigned,
                 [add_int (add_int ptr idx dbg) (Cconst_int 1) dbg], dbg) in
...
let unaligned_load_64 ptr idx dbg =
  assert(size_int = 8);
  if Arch.allow_unaligned_access
  then Cop(Cload Word_int, [add_int ptr idx dbg], dbg)
  else
    let v1 = Cop(Cload Byte_unsigned, [add_int ptr idx dbg], dbg) in
    let v2 = Cop(Cload Byte_unsigned,
                 [add_int (add_int ptr idx dbg) (Cconst_int 1) dbg], dbg) in
...
let rec transl env e =
  ...
  | Uifthenelse (cond', ifso', ifnot') ->
      ...
      | _cond, _ifso, _ifnot ->
          let dbg = Debuginfo.none in
          if_then_else(test_bool(transl env cond') dbg, transl env ifso',
            transl env ifnot')
      end
  | Usequence(exp1, exp2) ->
...
and transl_prim_1 env p arg dbg =
  ...
  | Pintoffloat ->
     tag_int(Cop(Cintoffloat, [transl_unbox_float env arg], dbg)) dbg
  | Pnegfloat ->
      box_float dbg (Cop(Cnegf, [transl_unbox_float env arg], dbg))
  | Pabsfloat ->
      box_float dbg (Cop(Cabsf, [transl_unbox_float env arg], dbg))
  (* String operations *)
  ...
  | Pintofbint bi ->
      force_tag_int (transl_unbox_int env bi arg) dbg
  | Pcvtbint(bi1, bi2) ->
      box_int dbg bi2 (transl_unbox_int env bi1 arg)
  | Pnegbint bi ->
      box_int dbg bi
        (Cop(Csubi, [Cconst_int 0; transl_unbox_int env bi arg], dbg))
  | Pbbswap bi ->
...
and transl_prim_2 env p arg1 arg2 dbg =
      ...
      return_unit(
        Cop(Cstore (Double_u, init),
            [if n = 0 then ptr
                   ...
                   transl_unbox_float env arg2], dbg))

  (* Boolean operations *)
  | Psequand ->
      if_then_else(test_bool(transl env arg1) dbg,
         ...
         (fun arg1 arg2 ->
           match arg1, arg2 with
             | Cconst_int _ as c1, c2 ->
                 incr_int (mul_int (untag_int c1 dbg) (decr_int c2 dbg) dbg)
                   dbg
             | c1, c2 ->
                 incr_int (mul_int (decr_int c1 dbg) (untag_int c2 dbg) dbg)
                   dbg)
     end
  | Pdivint ->
        ...
        (bind "str" (transl env arg1) (fun str ->
          bind "index" (untag_int (transl env arg2) dbg) (fun idx ->
            Csequence(
              ...
              Cop(Cload Byte_unsigned, [add_int str idx dbg], dbg))))) dbg
  ...
  | Pbigstring_load_16(unsafe) ->
     tag_int
       (bind "ba" (transl env arg1) (fun ba ->
        bind "index" (untag_int (transl env arg2) dbg) (fun idx ->
        bind "ba_data" (Cop(Cload Word_int, [field_address ba 1 dbg], dbg))
         (fun ba_data ->
  ...
  | Paddbint bi ->
      box_int dbg bi (Cop(Caddi,
                      [transl_unbox_int env bi arg1;
                       transl_unbox_int env bi arg2], dbg))
  | Psubbint bi ->
  ...
  | Plsrbint bi ->
      box_int dbg bi (Cop(Clsr,
                     [make_unsigned_int bi (transl_unbox_int env bi arg1) dbg;
                      untag_int(transl env arg2) dbg], dbg))
  | Pasrbint bi ->
  ...
  | Pbintcomp(bi, cmp) ->
      tag_int (Cop(Ccmpi(transl_comparison cmp),
                     [transl_unbox_int env bi arg1;
                      transl_unbox_int env bi arg2], dbg)) dbg
  | prim ->
            ...
            bind "index" (transl env arg2) (fun index ->
              bind "arr" (transl env arg1) (fun arr ->
                Cifthenelse(is_addr_array_ptr arr dbg,
                            ...
                            float_array_set arr index (unbox_float newval dbg)
                              dbg))))
      | Paddrarray ->
...
and transl_unbox_float env ulam =
  preserve_phantom_lets ulam (fun ulam ->
    ...
    | exp -> unbox_float(transl env exp) Debuginfo.none)
...
and transl_unbox_int env bi ulam =
  preserve_phantom_lets ulam (fun ulam ->
    ...
    | Uprim(Pbintofint bi', [arg], dbg) when bi = bi' ->
        preserve_phantom_lets arg (fun arg ->
          match arg with
          | Uconst (Uconst_int i) -> Cconst_int i
          | _ -> unbox_int bi (transl env ulam) dbg)
    | exp -> unbox_int bi (transl env exp) Debuginfo.none)
  ...
  preserve_phantom_lets cond (fun cond ->
    ...
    | Uifthenelse (cond, ifso, ifnot) ->
        make_catch2
          (fun shared ->
            if_then_else
              (test_bool (transl env cond) Debuginfo.none,
...
let send_function arity =
  ...
  let clos =
    ...
    Cifthenelse(Cop(Ccmpa Cne, [tag'; tag], dbg),
                ...
                cached_pos),
    Cop(Cload Word_val, [Cop(Cadda, [Cop (Cadda, [Cvar real; Cvar meths], dbg);
