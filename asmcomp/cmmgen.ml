...
let alloc_block_header tag sz dbg = Cblockheader(block_header tag sz, dbg)
...
let box_float dbg c = Cop(Calloc, [alloc_float_header dbg; c])
...
let make_alloc_generic set_fn dbg tag wordsize args =
  if wordsize <= Config.max_young_wosize then
    Cop(Calloc, Cblockheader(block_header tag wordsize, dbg) :: args)
  else begin
...
let rec is_unboxed_number = function
    Uconst(Uconst_ref(_, Uconst_float _)) ->
      Boxed_float Debuginfo.none
  | Uprim(p, _, dbg) ->
      begin match simplif_primitive p with
        | Pccall p ->
            if p.prim_native_float then Boxed_float dbg else No_unboxing
        | Pfloatfield _ -> Boxed_float dbg
        | Pfloatofint -> Boxed_float dbg
        | Pnegfloat -> Boxed_float dbg
        | Pabsfloat -> Boxed_float dbg
        | Paddfloat -> Boxed_float dbg
        | Psubfloat -> Boxed_float dbg
        | Pmulfloat -> Boxed_float dbg
        | Pdivfloat -> Boxed_float dbg
        | Parrayrefu Pfloatarray -> Boxed_float dbg
        | Parrayrefs Pfloatarray -> Boxed_float dbg
        | Pbintofint bi -> Boxed_integer(bi, dbg)
        | Pcvtbint(src, dst) -> Boxed_integer(dst, dbg)
        | Pnegbint bi -> Boxed_integer(bi, dbg)
        | Paddbint bi -> Boxed_integer(bi, dbg)
        | Psubbint bi -> Boxed_integer(bi, dbg)
        | Pmulbint bi -> Boxed_integer(bi, dbg)
        | Pdivbint bi -> Boxed_integer(bi, dbg)
        | Pmodbint bi -> Boxed_integer(bi, dbg)
        | Pandbint bi -> Boxed_integer(bi, dbg)
        | Porbint bi -> Boxed_integer(bi, dbg)
        | Pxorbint bi -> Boxed_integer(bi, dbg)
        | Plslbint bi -> Boxed_integer(bi, dbg)
        | Plsrbint bi -> Boxed_integer(bi, dbg)
        | Pasrbint bi -> Boxed_integer(bi, dbg)
        | Pbigarrayref(_, _, (Pbigarray_float32 | Pbigarray_float64), _) ->
            Boxed_float dbg
        | Pbigarrayref(_, _, Pbigarray_int32, _) -> Boxed_integer(Pint32, dbg)
        | Pbigarrayref(_, _, Pbigarray_int64, _) -> Boxed_integer(Pint64, dbg)
        | Pbigarrayref(_, _, Pbigarray_native_int,_) -> Boxed_integer(Pnativeint, dbg)
        | Pstring_load_32(_) -> Boxed_integer(Pint32, dbg)
        | Pstring_load_64(_) -> Boxed_integer(Pint64, dbg)
        | Pbigstring_load_32(_) -> Boxed_integer(Pint32, dbg)
        | Pbigstring_load_64(_) -> Boxed_integer(Pint64, dbg)
        | Pbbswap bi -> Boxed_integer(bi, dbg)
        | _ -> No_unboxing
      end
...
let rec transl = function
  ...
  | Ulet(id, exp, body) ->
      ...
      | Boxed_float dbg ->
          transl_unbox_let (box_float dbg) unbox_float transl_unbox_float
                           Double_u 0
                           id exp body
      | Boxed_integer(bi, dbg) ->
          transl_unbox_let (box_int dbg bi) (unbox_int bi) (transl_unbox_int bi)
  ...
  | Uprim(prim, args, dbg) ->
      ...
      | (Pmakeblock(tag, mut), args) ->
          make_alloc dbg tag (List.map transl args)
      | (Pccall prim, args) ->
          ...
          begin match kind with
            Pgenarray ->
              Cop(Cextcall("caml_make_array", typ_addr, true, Debuginfo.none),
                  [make_alloc dbg 0 (List.map transl args)])
