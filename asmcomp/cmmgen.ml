...
(* CR-someday mshinwell: A more satisfactory solution would be to change the Cmm
   ...
   [Cop]. This is probably not too difficult and would mean that the [Un_anf]
   pass could be removed entirely. *)
...
let lift_phantom_lets3 exp1 exp2 exp3 f =
  lift_phantom_lets exp1 (fun exp1 ->
    lift_phantom_lets exp2 (fun exp2 ->
      lift_phantom_lets exp3 (fun exp3 ->
        f exp1 exp2 exp3))

let lift_phantom_lets_lift exp_list f =
  List.map (fun exp -> lift_phantom_lets exp f) exp_list

let lift_phantom_lets_clambda exp f =
  let rec lift exp lifted_rev depth =
    let next_depth = depth + 1 in
    match exp with
  ...
  let exp = f exp in
  List.fold_left (fun exp (var, defining_expr) ->
      Uphantom_let (var, defining_expr, exp))
    exp
    lifted_phantom_lets_rev

let lift_phantom_lets2_clambda exp1 exp2 f =
  lift_phantom_lets_clambda exp1 (fun exp1 ->
    lift_phantom_lets_clambda exp2 (fun exp2 ->
      f exp1 exp2))
...
let rec unbox_float dbg cmm =
  lift_phantom_lets cmm (fun cmm ->
    ...
    | Ctrywith(e1, id, e2) ->
      Ctrywith(unbox_float dbg e1, id, unbox_float dbg e2)
...
let rec unbox_int bi arg dbg =
  lift_phantom_lets arg (fun arg ->
    match arg with
      Cop(Calloc, [_hdr; _ops; Cop(Clsl, [contents; Cconst_int 32], dbg')],
        _dbg)
...
struct
  ...
  let make_catch handler =
    lift_phantom_lets handler (fun handler ->
      | Cexit (i,[]) -> i,fun e -> e
      | _ ->
...
let rec transl env e =
  ...
  | Uprim(prim, args, dbg) ->
      ...
      | (Pduparray (kind, _), args) ->
          lift_phantom_lets_list args (fun args ->
            match args with
  ...
  | Uifthenelse (cond, ifso, ifnot) ->
      lift_phantom_lets3 cond ifso ifnot (fun cond ifso ifnot ->
        match cond, ifso, ifnot with
        | Uprim(Pnot, [arg], _), ifso, ifnot ->
            transl env (Uifthenelse(arg, ifnot, ifso))
        | cond, ifso, Ustaticfail (nfail, []) ->
            let dbg = Debuginfo.none in
            exit_if_false dbg env cond (transl env ifso) nfail
        | cond, Ustaticfail (nfail, []), ifnot ->
            let dbg = Debuginfo.none in
            exit_if_true dbg env cond nfail (transl env ifnot)
        | Uprim(Psequand, _, dbg) as cond, ifso, ifnot ->
            let raise_num = next_raise_count () in
            make_catch
              raise_num
              (exit_if_false dbg env cond (transl env ifso) raise_num)
              (transl env ifnot)
        | Uprim(Psequor, _, dbg) as cond, ifso, ifnot ->
            let raise_num = next_raise_count () in
            make_catch
              raise_num
              (exit_if_true dbg env cond raise_num (transl env ifnot))
              (transl env ifso)
        | Uifthenelse (cond, condso, condnot), ifso, ifnot ->
            let dbg = Debuginfo.none in
     ...
     begin
       (* decrementing the non-constant part helps when the multiplication is
          ...
          rather than
            (+ ( * 200 (>>s a 1)) 15)
        *)
       lift_phantom_lets (transl env arg1) (transl env arg2)
         (fun arg1 arg2 ->
...
and transl_unbox_float dbg env ulam =
  lift_phantom_lets_clambda ulam (fun ulam ->
    match ulam with
...
and exit_if_true dbg env cond nfail otherwise =
  lift_phantom_lets_clambda cond (fun cond ->
    match cond with
...
and exit_if_false dbg env cond otherwise nfail =
  lift_phantom_lets_clambda2 cond otherwise (fun cond otherwise ->
    match cond with
...
let cache_public_method meths tag cache dbg =
  ...
  Clet (
  Ident_ibp.create hi None, Cop(Cload (Word_int, Mutable), [meths], dbg),
  Csequence(
  ccatch
    (raise_num, [],
     Cloop
...
let send_function arity =
  ...
  let clos =
    ...
    Clet (
    Ident_ibp.create meths None, Cop(Cload (Word_val, Mutable), [obj], dbg),
    Clet (
    ...
    ::
      (if arity <= max_arity_optimized && arity - num > 2 then
          ...
          let rec iter i args clos =
            if i = 0 then
              Cop(Capply typ_val,
                  (get_field env (Cvar clos) 2 dbg) :: args @ [Cvar clos], dbg)
            else
              let newclos = Ident.create "clos" in
              Clet(Ident_ibp.create newclos None,
                   get_field env (Cvar clos) 4 dbg,
