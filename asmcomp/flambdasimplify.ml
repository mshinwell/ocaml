...
let remove_unused_closure_variables expr =
  let all_used_vars_defined_within_closures =
    Flambdautils.all_used_vars_defined_within_closures expr
  in
        ...
        Variable.Map.filter (fun id _ ->
            Variable.Set.mem id all_free_vars
              || Var_within_closure.Set.mem (Var_within_closure.wrap id)
                  all_used_vars_defined_within_closures)
          cl_free_var
      in
      Fset_of_closures ({ closure with cl_free_var = cl_free_vars }, eid)
    | expr -> expr
  in
  Flambdaiter.map aux expr
