...
let rec with_afl_logging b =
  ...
  let instrumentation =
    ...
    Clet(afl_area, op (Cload (Word_int, Mutable)) [afl_area_ptr],
    Clet(cur_pos,  op Cxor [op (Cload (Word_int, Mutable))
      [afl_prev_loc]; Cconst_int cur_location],
    Csequence(
      op (Cstore(Byte_unsigned, Assignment))
         [op Cadda [Cvar afl_area; Cvar cur_pos];
          op Cadda [op (Cload (Byte_unsigned, Mutable))
                       [op Cadda [Cvar afl_area; Cvar cur_pos]];
                    Cconst_int 1]],
...
and instrument = function
  ...
  | Cblockheader _ | Cvar _ | Cunreachable as c -> c
