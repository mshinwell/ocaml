       ...
       (List.map
          (fun {mb_id=id; mb_name; mb_expr=modl; mb_loc=loc; _} ->
             let id_or_ignore_loc, shape =
               match id with
               | None ->
                 let loc = of_raw_location ~scopes mb_name.loc in
...
let merge_inline_attributes attr1 attr2 loc =
  ...
  | Some attr -> attr
  | None -> raise (Error (raw_location loc, Conflicting_inline_attributes))
...
let merge_functors ~scopes mexp coercion root_path =
  let rec merge ~scopes mexp coercion path acc inline_attribute =
    ...
    | Tmod_functor (param, body) ->
      ...
      in
      let loc = of_raw_location ~scopes mexp.mod_loc in
      let path, param =
        match param with
                      ...
                      match cc with
                        Tcoerce_primitive p ->
                          Translprim.transl_primitive
                            (of_raw_location ~scopes p.pc_loc)
      ...
      | Tstr_module ({mb_presence=Mp_present} as mb) ->
          ...
          | None ->
              Lsequence (Lprim(Pignore, [module_body],
                               of_raw_location ~scopes mb.mb_name.loc), body),
              size
          | Some id ->
              let module_body =
                Levent (module_body, {
                  lev_loc = of_raw_location ~scopes mb.mb_loc;
            ...
            | id :: ids ->
                ...
                Llet(Alias, Pgenval, id,
                     Lprim(Pfield pos, [Lvar mid],
                           of_raw_location ~scopes incl.incl_loc), body),
                size
          in
    ...
    | item :: rem ->
        ...
        | Tstr_module
            ...
            let lam =
              Translattribute.add_inline_attribute
                ...
                loc mb_attributes
            in
            Lsequence(Lprim(Pignore,[lam],of_raw_location ~scopes mb_name.loc),
                      transl_store ~scopes rootpath subst cont rem)
        ...
        | Tstr_module
            ...
            Llet(Strict, Pgenval, id, Lambda.subst no_env_update subst lam,
                 Lsequence(store_ident (of_raw_location ~scopes loc) id,
                           transl_store ~scopes rootpath
                             (add_ident true id subst)
                             cont rem))
        | Tstr_module {mb_presence=Mp_absent} ->
        ...
        | Tstr_include{
            ...
            in
            let subst = !transl_store_subst in
            let field = field_of_str (of_raw_location ~scopes loc) str in
        ...
        | Tstr_open od ->
            ...
            | Tmod_structure str ->
                ...
                in
                let loc = of_raw_location ~scopes od.open_loc in
                let ids = Array.of_list (defined_idents str.str_items) in
