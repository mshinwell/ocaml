...
let transl_extension_constructor ~scopes env path ext =
  ...
  match ext.ext_kind with
    Text_decl _ ->
      Lprim (Pmakeblock (Obj.object_tag, Mutable, None),
...
let event_before ~scopes exp lam =
  Translprim.event_before (of_raw_location ~scopes exp.exp_loc) exp lam

let event_after ~scopes exp lam =
  Translprim.event_after (of_raw_location ~scopes exp.exp_loc) exp lam
...
let event_function ~scopes exp lam =
  if !Clflags.debug && not !Clflags.native_code then
    ...
    (info,
     Levent(body, {lev_loc = of_raw_location ~scopes exp.exp_loc;
...
let assert_failed ~scopes exp =
  ...
  in
  let loc = of_raw_location ~scopes exp.exp_loc in
...
let transl_ident loc env ty path desc =
  ...
  | Val_anc _ ->
      raise(Error(raw_location loc, Free_super_var))
  | Val_reg | Val_self _ ->
...
and transl_exp0 ~scopes e =
  ...
  | Texp_ident(path, _, desc) ->
      transl_ident (of_raw_location ~scopes e.exp_loc)
    ...
    && List.for_all (fun (_, arg) -> arg <> None) oargs ->
      ...
      let lam =
        Translprim.transl_primitive_application
          (of_raw_location ~scopes e.exp_loc) p e.exp_env prim_type path
      ...
      if extra_args = [] then lam
        ...
        event_after ~scopes e
          (transl_apply ~scopes ~should_be_tailcall ~inlined ~specialised
             lam extra_args (of_raw_location ~scopes e.exp_loc))
      end
  | Texp_apply(funct, oargs) ->
      ...
      event_after ~scopes e
        (transl_apply ~scopes ~should_be_tailcall ~inlined ~specialised
           (transl_exp ~scopes funct) oargs (of_raw_location ~scopes e.exp_loc))
  ...
  | Texp_tuple el ->
      ...
      with Not_constant ->
        Lprim(Pmakeblock(0, Immutable, Some shape), ll,
              (of_raw_location ~scopes e.exp_loc))
      end
  | Texp_construct(_, cstr, args) ->
      ...
      | Cstr_extension(path, is_const) ->
          let lam = transl_extension_path
                      (of_raw_location ~scopes e.exp_loc) e.exp_env path in
          if is_const then lam
          else
            Lprim(Pmakeblock(0, Immutable, Some (Pgenval :: shape)),
                  lam :: ll, of_raw_location ~scopes e.exp_loc)
      end
  | Texp_extension_constructor (_, path) ->
      transl_extension_path (of_raw_location ~scopes e.exp_loc) e.exp_env path
  | Texp_variant(l, arg) ->
  ...
  | Texp_letmodule(None, loc, Mp_present, modl, body) ->
      ...
      Lsequence(Lprim(Pignore, [lam], of_raw_location ~scopes loc.loc),
  ...
  | Texp_lazy e ->
      ...
      | `Float_that_cannot_be_shortcut ->
          ...
          Lprim(Pmakeblock(Obj.forward_tag, Immutable, None),
                [transl_exp ~scopes e], of_raw_location ~scopes e.exp_loc)
      ...
      | `Other ->
         ...
         let fn = Lfunction {kind = Curried;
                             ...
                             loc = of_raw_location ~scopes e.exp_loc;
            ...
            List.fold_left (fun (body, pos) id ->
              Llet(Alias, Pgenval, id,
                   Lprim(Pfield (field_info pos, Reads_agree), [Lvar oid],
                         of_raw_location ~scopes od.open_loc), body),
              pos + 1
  ...
  match cases with
    ...
    when Parmatch.inactive ~partial pat ->
      ...
      ((Curried, (param, kind) :: params, return),
       Matching.for_function (of_raw_location ~scopes loc) None (Lvar param)
         [pat, body] partial)
  | {c_lhs={pat_desc = Tpat_tuple pl}} :: _ when untuplify_fn ->
      begin try
        ...
        ((Tupled, tparams, return),
         Matching.for_tupled_function (of_raw_location ~scopes loc) params
  ...
  match rec_flag with
    Nonrecursive ->
      ...
      | {vb_pat=pat; vb_expr=expr; vb_attributes=attr; vb_loc} :: rem ->
          ...
          fun body -> Matching.for_let loc lam pat (mk_body body)
...
and transl_match ~scopes e arg pat_expr_list partial =
  ...
  let classic =
    ...
    | {exp_desc = Texp_tuple argl}, [] ->
      ...
      Matching.for_multiple_match (of_raw_location ~scopes e.exp_loc)
    ...
    | {exp_desc = Texp_tuple argl}, _ :: _ ->
        ...
        static_catch (transl_list ~scopes argl) val_ids
          (Matching.for_multiple_match
             (of_raw_location ~scopes e.exp_loc) lvars val_cases partial)
    | arg, [] ->
      assert (static_handlers = []);
      Matching.for_function (of_raw_location ~scopes e.exp_loc)
...
and transl_letop ~scopes loc env let_ ands param case partial =
  let rec loop prev_lam = function
    ...
    | and_ :: rest ->
        ...
        let op =
          transl_ident (of_raw_location ~scopes and_.bop_op_name.loc) env
        ...
        let lam =
          bind Strict right_id exp
            (Lapply{ap_should_be_tailcall = false;
                    ap_loc = of_raw_location ~scopes and_.bop_loc;
                    ap_func = op;
  ...
  in
  let op =
    transl_ident (of_raw_location ~scopes let_.bop_op_name.loc) env
  ...
  let func =
    ...
    in
    let attr = default_function_attribute in
    let loc = of_raw_location ~scopes case.c_rhs.exp_loc in
    Lfunction{kind; params; return; body; attr; loc}
  in
  Lapply{ap_should_be_tailcall = false;
         ap_loc = of_raw_location ~scopes loc;
         ap_func = op;
         ap_args=[exp; func];
         ap_inlined=Default_inline;
         ap_specialised=Default_specialise}
