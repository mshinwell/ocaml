                                               ...
                                               intnat profinfo)
{
  ...
  if (caml_allocation_tracing) {
    ...
    count =
      (uint64_t*) (((uint64_t) caml_major_allocation_tracing_array)
        + caller_aligned);
    if (count < caml_major_allocation_tracing_array_end) {
  ...
  if (caml_allocation_tracing) {
    ...
    uint64_t limit =
      (((uint64_t) caml_minor_allocation_tracing_array_end)
        - ((uint64_t) caml_minor_allocation_tracing_array)) / sizeof(uint64_t);
    for (i = 0; i < limit; i++) {
      caml_minor_allocation_tracing_array[i] = 0;
      caml_major_allocation_tracing_array[i] = 0;
    }
  }
  return v_unit;
}
