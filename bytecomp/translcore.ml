...
let check_recursive_lambda idlist lam =
  let rec check_top idlist = function
    ...
    | Lletrec(bindings, body) ->
        ...
        List.for_all (fun (id, _attributes, arg) -> check idlist' arg) bindings &&
  ...
  and add_letrec bindings idlist =
    List.fold_right (fun (id, _attributes, arg) idl -> add_let id arg idl)
                    bindings idlist
      ...
      let idlist =
        List.map
          (fun {vb_pat=pat} -> match pat.pat_desc with
              Tpat_var (id,_) -> id, pat.pat_attributes
            | Tpat_alias ({pat_desc=Tpat_any}, id,_) -> id, pat.pat_attributes
            | _ -> raise(Error(pat.pat_loc, Illegal_letrec_pat)))
        pat_expr_list in
      let transl_case {vb_pat=pat; vb_expr=expr} (id, attributes) =
        let lam = transl_exp expr in
        if not (check_recursive_lambda (List.map fst idlist) lam) then
          raise(Error(expr.exp_loc, Illegal_letrec_expr));
        (id, attributes, lam) in
