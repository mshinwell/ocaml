...
let rec apply_coercion strict restr arg =
  ...
  | Tcoerce_functor(cc_arg, cc_res) ->
      ...
      let carg = apply_coercion Alias cc_arg (Lvar param) in
...
and apply_coercion_result loc strict funct params args cc_res =
  ...
  | Tcoerce_functor(cc_arg, cc_res) ->
    let param = Ident.create "funarg" in
    let arg = apply_coercion Alias cc_arg (Lvar param) in
  ...
  let params, body =
    List.fold_left (fun (params, body) (param, loc, arg_coercion) ->
        ...
        let arg =
          ...
          | None -> Lvar param'
          | Some arg_coercion ->
            apply_coercion Alias arg_coercion (Lvar param')
        in
        let params = param' :: params in
        let body = Llet (Alias, param, arg, body) in
        params, body)
  ...
  | _ ->
      ...
      | Tmod_functor _ ->
          oo_wrap mexp.mod_env true (fun () ->
            compile_functor mexp cc rootpath mexp.mod_loc) ()
